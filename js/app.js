...
const util = (() => {
    ...
    const animation = async () => {
        ...
        (async function frame() {
            ...
            await confetti({
                ...
                colors: ["FFC0CB", "FF69B4", "FF1493", "C71585", "FF0000", "FF6600"],
                shapes: ["heart"],
    ...
    const buka = async (button) => {
        ...
        opacity('welcome');
        document.getElementById('web-music').style.display = 'block';
        timer();
...
const audio = (() => {
    ...
    const singleton = () => {
        if (!audio) {
            audio = new Audio();
            audio.src = document.getElementById('web-music').getAttribute('data-url');
            audio.load();
            audio.currentTime = 0;
            audio.autoplay = true;
            audio.muted = false;
...
const pagination = (() => {
    ...
    const buttonAction = async (button) => {
        ...
        document.getElementById('daftar-ucapan').scrollIntoView({
            behavior: 'smooth'
        });
...
const like = (() => {
    ...
    const like = async (button) => {
        ...
        if (!token) {
            alert('There is an error, token kosong !');
...
const comment = (() => {
    ...
    const ucapan = async () => {
        ...
        let token = localStorage.getItem('token') || '';
        if (token.length == 0) {
            alert('There is an error, empty token !');
            window.location.reload();
            return;
        }
        ...
        await request('GET', `/api/comment?per=${pagination.getPer()}&next=${pagination.getNext()}`)
            ...
            .catch((err) => alert(`There is an error 6: ${err}`));
    };
    ...
    const edit = async (button) => {
        ...
        await request('GET', '/api/comment/' + id)
            ...
            .catch((err) => {
                alert(`There is an error 10: ${err}`);
            });
    ...
    const batal = () => {
        if (temporaryID) {
            document.getElementById(temporaryID).scrollIntoView({
                behavior: 'smooth',
                block: 'center'
            });
            temporaryID = null;
